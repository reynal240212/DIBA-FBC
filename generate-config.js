...
// Leer variables de entorno de Vercel (con fallbacks estandar de Next.js/Vercel)
const SUPABASE_URL = process.env.SUPABASE_URL || process.env.NEXT_PUBLIC_SUPABASE_URL;
const SUPABASE_ANON_KEY = process.env.SUPABASE_ANON_KEY || process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY;

console.log('--- Verificación de Entorno ---');
console.log('SUPABASE_URL:', SUPABASE_URL ? '✅ Detectada' : '❌ NO DETECTADA');
console.log('SUPABASE_ANON_KEY:', SUPABASE_ANON_KEY ? '✅ Detectada' : '❌ NO DETECTADA');

// Validar que las variables existen
if (!SUPABASE_URL || !SUPABASE_ANON_KEY) {
    console.error('\n❌ ERROR CRÍTICO: Las variables de entorno son requeridas para el build.');
    console.error('👉 Intenta agregarlas en Vercel con el prefijo "NEXT_PUBLIC_":');
