...
// Correcta inicialización del cliente de Supabase
const { createClient } = supabase; // Destructura la función del objeto global 'supabase'
...
const showToast = (message, title = 'Notificación', type = 'success') => {
    ...
    toastEl.className = 'toast'; // Limpiar clases de color previas
...
const views = {
    dashboard: async () => {
        ...
        view.innerHTML = `
            ...
            <div class="row g-4">
                <div class="col-lg-4 col-md-6"><div class="stats-card card card-body h-100"><div class="d-flex align-items-center"><div class="icon me-3"><i class="fas fa-users"></i></div><div><h5 class="card-title">Total Jugadores</h5><h3 class="mb-0">${playerError ? 'Error' : playerCount}</h3></div></div></div></div>
                <div class="col-lg-4 col-md-6"><div class="stats-card card card-body h-100 accent-card"><div class="d-flex align-items-center"><div class="icon me-3"><i class="fas fa-clipboard-list"></i></div><div><h5 class="card-title">Planillas Creadas</h5><h3 class="mb-0">${planillaError ? 'Error' : planillaCount}</h3></div></div></div></div>
            </div>`;
        hideLoading();
    },
    ...
    players: async () => {
        ...
        view.innerHTML = `
            <div class="d-flex justify-content-between align-items-center mb-4"><h1 class="display-6">Gestión de Jugadores</h1><button id="newPlayerBtn" class="btn btn-success"><i class="fas fa-user-plus me-1"></i> Nuevo Jugador</button></div>
            <div class="card card-body"><div class="table-responsive"><table id="playersTable" class="table table-striped table-hover w-100"><thead><tr><th>ID</th><th>Nombre</th><th>Categoría</th><th>Nacimiento</th><th>Estado</th><th>Acciones</th></tr></thead><tbody></tbody></table></div></div>`;
    ...
    planillas: async () => {
        ...
        view.innerHTML = `
            <div class="d-flex justify-content-between align-items-center mb-4"><h1 class="display-6">Gestión de Planillas</h1><button id="newPlanillaBtn" class="btn btn-success"><i class="fas fa-plus me-1"></i> Nueva Planilla</button></div>
            <div class="card card-body"><div class="table-responsive"><table id="planillasTable" class="table table-hover w-100"><thead><tr><th>ID</th><th>Fecha</th><th>Categoría</th><th>Acciones</th></tr></thead><tbody></tbody></table></div></div>`;
    ...
    planillaDetail: async (planillaId) => {
        ...
        view.innerHTML = `
            <div class="mb-4"><button id="backToPlanillasBtn" class="btn btn-outline-primary mb-3"><i class="fas fa-arrow-left me-1"></i> Volver a Planillas</button><h1 class="display-6">Planilla: ${formatDate(planilla.fecha)}</h1><p class="lead">Categoría: <strong>${planilla.categoria}</strong></p></div>
            <div class="card card-body"><div class="table-responsive"><table id="planillaDetailsTable" class="table w-100"><thead><tr><th>Jugador</th><th>Pago</th><th>Asistencia</th><th>Observaciones</th></tr></thead><tbody></tbody></table></div></div>`;
...
async function loadPlayersData() {
    ...
    document.querySelector("#playersTable tbody").innerHTML = data.map(p => `
        <tr data-id="${p.id}">
            <td>${p.id}</td><td>${p.nombre_completo}</td><td>${p.categoria}</td><td>${formatDate(p.fecha_nacimiento)}</td>
            <td><span class="badge ${p.status === 'Activo' ? 'bg-success' : 'bg-danger'}">${p.status}</span></td>
            <td><button class="btn btn-sm btn-primary edit-btn" title="Editar"><i class="fas fa-edit"></i></button> <button class="btn btn-sm btn-danger delete-btn" title="Eliminar"><i class="fas fa-trash"></i></button></td>
        </tr>`).join('');
...
async function loadPlanillasData() {
    ...
    document.querySelector("#planillasTable tbody").innerHTML = data.map(p => `
        <tr data-id="${p.id}">
            <td>${p.id}</td><td>${formatDate(p.fecha)}</td><td>${p.categoria}</td>
            <td><button class="btn btn-sm btn-primary view-btn"><i class="fas fa-eye"></i> Ver</button> <button class="btn btn-sm btn-danger delete-planilla-btn"><i class="fas fa-trash"></i> Eliminar</button></td>
        </tr>`).join('');
...
async function createPlanilla(event) {
    event.preventDefault(); showLoading();
    const { data, error } = await supabaseClient.from('planillas').insert({
...
async function loadPlanillaDetails(planillaId, categoria) {
    ...
    document.querySelector('#planillaDetailsTable tbody').innerHTML = jugadores.map(jugador => {
        ...
        <tr data-jugador-id="${jugador.id}">
            ...
            <td><select class="form-select form-select-sm data-input status-select-${asistencia}" data-field="asistencia"><option value="P" ${asistencia === 'P' ? 'selected' : ''}>Presente</option><option value="A" ${asistencia === 'A' ? 'selected' : ''}>Ausente</option><option value="E" ${asistencia === 'E' ? 'selected' : ''}>Excusa</option></select></td>
            <td><input type="text" class="form-control form-control-sm data-input" data-field="observacion" value="${registro.observacion || ''}" placeholder="..."></td>
        </tr>`;
    }).join('');
}
...
async function saveRegistroChange(event, planillaId) {
    ...
    const { error } = await supabaseClient.from('planilla_registros').upsert({
        planilla_id: planillaId, jugador_id: jugadorId, [input.dataset.field]: input.value
...
document.addEventListener('DOMContentLoaded', () => {
    supabaseClient.auth.onAuthStateChange((event, session) => {
        ...
        if (session && !document.querySelector('.view[style*="display: block"]')) navigateTo('dashboard');
        hideLoading();
    });
    ...
    document.body.addEventListener('change', (e) => {
        if (e.target.matches('#planillaDetailsTable .data-input')) {
            saveRegistroChange(e, e.target.closest('#planilla-detail-view').querySelector('h1').parentElement.dataset.planillaId);
        }
    });
