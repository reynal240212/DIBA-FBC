/**
 * DIBA FBC - Gestor Storage Module (Supabase Admin Style)
 */
...
export async function loadStorageBuckets(dynamicContent, pageTitle, mainTitle, setActiveFilter) {
    ...
    try {
        ...
        buckets.forEach(bucket => {
            ...
            div.className = "panel p-5 group hover:border-gold/30 transition-all cursor-pointer flex items-center relative overflow-hidden";
            div.innerHTML = `
            ...
            if (rowsContainer) rowsContainer.appendChild(div);
        });
    } catch (err) {
        console.error(err);
        const rowsContainer = document.getElementById('buckets-rows');
                ...
                <div class="p-6 border-b border-white/5 flex items-center justify-between flex-wrap gap-4">
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <div class="flex bg-white/5 p-1 rounded-xl">
                            <button class="w-8 h-8 rounded-lg bg-gold text-[#004d98] flex items-center justify-center"><i class="fas fa-grid-2 text-xs"></i></button>
