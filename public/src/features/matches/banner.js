...
export async function initMatchBanner() {
    ...
    try {
        const response = await fetch('http://localhost:8080/api/banner');
        if (!response.ok) throw new Error('Error en la respuesta del servidor');
        
        const { partidos, esFuturo, proximaFechaLabel } = await response.json();
        ...
        if (esFuturo) {
            ...
            // Usamos la fecha del primer partido ya formateada por el backend
            title.innerHTML = `PRÓXIMOS PARTIDOS: ${partidos[0].fecha}`;
        ...
        if (loadingStatus) loadingStatus.innerHTML = `<span class="text-red-400 text-xs">Error cargando desde API.</span>`;
    }
}
    ...
    card.innerHTML = `
        ...
        <div class="flex items-center justify-between gap-4 w-full relative z-10">
            <div class="flex flex-col items-center w-[35%]">
                <div class="relative mb-3">
                    ...
                    <img src="${p.escudoLocal}" class="w-14 h-14 object-contain relative z-10 drop-shadow-[0_0_15px_rgba(255,255,255,0.1)] group-hover/card:scale-110 transition-transform duration-500" onerror="this.src='/images/ESCUDO.png'">
                </div>
                <span class="text-white font-black text-[10px] sm:text-[11px] uppercase text-center line-clamp-2 leading-tight tracking-tight">${p.equipoLocal}</span>
            </div>
            <div class="flex flex-col items-center justify-center w-[30%] px-1">
                ...
                <span class="text-slate-400 text-[9px] font-bold text-center w-full truncate tracking-tighter uppercase opacity-60">${p.cancha}</span>
            </div>
            <div class="flex flex-col items-center w-[35%]">
                <div class="relative mb-3">
                    <div class="absolute inset-0 bg-white/5 rounded-full blur-xl scale-150 group-hover/card:bg-white/10 transition-all duration-700"></div>
                    <img src="${p.escudoVisitante}" class="w-14 h-14 object-contain relative z-10 drop-shadow-[0_0_15px_rgba(255,255,255,0.1)] group-hover/card:scale-110 transition-transform duration-500" onerror="this.src='/images/ESCUDO.png'">
                </div>
                <span class="text-white font-black text-[10px] sm:text-[11px] uppercase text-center line-clamp-2 leading-tight tracking-tight">${p.equipoVisitante}</span>
            </div>
        </div>
        ...
        <div class="mt-2 flex items-center justify-between gap-4">
            <div class="flex items-center gap-1.5 opacity-40">
                ...
                <span class="text-[9px] font-bold text-white uppercase italic tracking-tighter">${p.fecha}</span>
            </div>
