...
export async function loadStorageBuckets(dynamicContent, pageTitle, mainTitle, setActiveFilter) {
    ...
    try {
        ...
        buckets.forEach(bucket => {
            ...
            card.innerHTML = `
                <div class="w-12 h-12 ${currentBucket === bucket.id ? 'bg-dibaGold text-blue-900' : 'bg-slate-50 text-slate-400'} rounded-2xl flex items-center justify-center text-xl transition-all">
                    <i class="fas fa-archive"></i>
                </div>
                <div><h4 class="font-black text-slate-800 text-xs uppercase italic tracking-tight">${bucket.name}</h4></div>
            `;
...
export async function loadFilesFromBucket(bucketId, path = '') {
    ...
    try {
        ...
        files.forEach(file => {
            ...
            div.className = "p-6 bg-white border border-slate-200 rounded-[2rem] hover:shadow-xl transition-all animate__animated animate__fadeIn group";
            div.innerHTML = `
                <div class="flex items-center gap-4 mb-4">
                    <div class="w-12 h-12 ${isFolder ? 'bg-blue-50 text-dibaBlue' : 'bg-slate-50 text-slate-400'} rounded-2xl flex items-center justify-center"><i class="fas ${isFolder ? 'fa-folder' : 'fa-file-alt'} text-xl"></i></div>
                    <h4 class="font-black text-slate-800 text-xs uppercase italic">${file.name}</h4>
                </div>
                <div class="pt-4 border-t border-slate-50 mt-auto">
                    ${isFolder ? `<button onclick="window.openSubFolder('${fullPath}')" class="w-full py-3 bg-dibaBlue text-white rounded-xl">Abrir</button>` : `<a href="${fileUrl}" target="_blank" class="w-full py-3 border border-slate-200 block text-center rounded-xl">Ver</a>`}
                </div>
            `;
